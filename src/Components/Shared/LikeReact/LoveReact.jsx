...
const LoveReact = ({ lessonId }) => {
  ...
  const { data = { liked: false, totalLikes: 0 }, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const url =
        `${import.meta.env.VITE_API_URL}/loveReact/${lessonId}` +
  ...
  const handleToggle = () => {
    ...
    setPlay(lessonId); // trigger Lottie animation
    mutation.mutate();
  };
  ...
  return (
    <div className="flex items-center space-x-2">
      <button
        ...
        className={`p-1 rounded-full transition-colors duration-150 cursor-pointer focus:outline-none 
      ...
      >
        ...
        {data.liked ? (
          ...
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" className="w-6 h-6">
      ...
      {play === lessonId && (
        <Lottie
          ...
          className="w-[500px] h-[500px] fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2  z-50 cursor-pointer"
        />
      )}
